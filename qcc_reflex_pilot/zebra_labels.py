...
def prepare_label_context(
    ...
    lot = _clean_text(lot_number or package.get("source_harvest_names", ""), 42)
    if not strain:
...
^FT82,248^A0B,16,13^FDTotal Cannabinoids: {_pct(a.get('total_cannabinoids'))}^FS
^FT103,248^A0B,16,13^FDTotal Terpenes: {_pct(a.get('total_terpenes'))}^FS
